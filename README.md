...
# Style Isolation
The classes generated by makeStyles are randomly generated, and basically there will be no duplicate names. Of course, you can also pass the current component name to the second parameter of `makeStyles`, then the currently created class is only used in the current component.
```JS
  // ...
...
```
You can uniformly configure your css common styles in your `theme.js` file, and bind with the name you define in `componentName` in the makeStyles function
```JS
  export default createTheme({
    // ...
    css: {
...
```
> Warning: The styles defined here take precedence over the ones you define in the component
